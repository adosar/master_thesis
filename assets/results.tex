...
Figure \ref{fig:learning_curves_mofs}, in the \gls{mof}-\ch{CO2} case, the
...
data} and retraining from scratch the \gls{cnn} for every thermodynamic
...
algorithm\index{Deep learning algorithm} under the hood, opens the door for
applying \emph{\textbf{transfer learning} techniques}\index{Transfer learning}.
...
\gls{nn} on an original task---known as the \emph{source task}\index{Source
task}---may require less training data to perform well on a new task---known as
the \emph{target task}\index{Target task}---if there is some \emph{similarity between
...
data.} and then fine-tune\index{Fine-tune} this pre-trained
model\index{Pre-trained model} on the other conditions.
...
and \emph{crystal graphs}\index{Crystal graphs}. The main drawback of these
...
set of coordinates\index{Coordinates} and associated features}. In our context,
...
not another mathematical representation\index{Representation} of a material---in
the sense of a descriptor\index{Descriptor}---it is the material
