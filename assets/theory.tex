...
\begin{theorem}[Bias-variance decomposition\index{Bias-variance decomposition}]
...
number of training samples increase, the training accuracy remains high since
...
learner's performance as function of experience and complexity.
...
position to dive into \gls{dl}. One might expect that \gls{dl} is a very
complex subfield of \gls{ml}, given its astonishing results in complex tasks,
but quite the opposite holds. Before exploring \glspl{nn}, the backbone of
\gls{dl}, we first need to understand how the basic building block of \glspl{nn}
, known as the \emph{\textbf{neuron}}\index{Neuron}, works.

\emph{A neuron is nothing more than a device---a simple computational
unit---that makes decisions by weighing up evidence} \parencite{Nielsen2018}.
This sounds very similar to the way humans make decisions. For instance,
suppose the weekend is coming up and your favorite singer has scheduled a
concert near your city. In order to decided whether you should go to the
concert or not, you might weigh up different factors, such as weather
...
\end{equation}
If this weighted sum\index{Weighted sum} plus the bias\index{bias}---your
willing to go to the festival irrespective of the evidence---is greater than
zero, then your decision is positive, otherwise negative.

\begin{itemize}
	\item Start with the neuron. Describes what the neuron does.
	\item Move to neural networks.
	\item Describe what the the neural does (feature extraction) with an example
		(image classification)
	\item Universal approximation theorem and no free-lunch theorem
	\item Define what we meen by deep learning (composition of functions)
\end{itemize}
...
\begin{figure}
	...
	\begin{subfigure}[b]{0.49\textwidth}
		\begin{tikzpicture}
			...
			\end{axis}
		\end{tikzpicture}
		\caption{Some non-linearities.}
	\end{subfigure}
	\caption{The perceptron and non-linearities.}
