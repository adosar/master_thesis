...
\colon \mcl{X} \to \mcl{Y}$, called the \emph{hypothesis}\index{Hypothesis},
...
forming the \emph{\textbf{training set}}\index{Training set} denoted as
...
nonlinearityity\index{Non-linearity}}. As such, we can rewrite \Equation{}
...
values of the activation function $\phi$. If these are smaller than one, then
...
are enough to obtain the gradient.
...
\begin{figure}
	...
	optimization\index{Optimization}. Finding good model parameters is
	essentially an optimization problem. From a purely optimization perspective,
	the blue solution is the ideal. In contrast, from a generalization
	point of view the red solution is preferred since it has lower error on new
	unseen samples.}
...
training set or just one sample, in the case of \gls{sgd}. The insight of these
algorithms is that \emph{the gradient over the whole training set is an
expectation and as such, it may be approximately estimated using only a small
...
hyperparameter\index{Hyperparamter} $\beta \in (0, 1)$, which is typically set to
\num{0.9}. By averaging past gradients, their zig-zag directions (see
\Figure{} \ref{fig:gd_variants}) effectively cancels out, leading to a smoother
...
symbol $\odot$ represents the element-wise multiplication wheres $\oplus$
...
\begin{algorithm}[H]
	...
	\While{stopping criterion not met}{
		...
		$\vc{\widehat{m}} \gets \frac{\vc{m}}{1 - w_1^t}$\;
		$\vc{\widehat{s}} \gets \frac{\vc{s}}{1 - w_2^t}$\;
