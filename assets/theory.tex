...
called the \emph{hypothesis}\index{Hypothesis}, which produces an output $\vcy
\in Y$ given an input $\vcx \in X$. At our disposal we have a collection of
input-output pairs $(\vcx_i, \vcy_i)$, forming the \emph{training set}\index{Training set}
$\dtrain$, with the pairs drawn \acrshort{iid} from $p(\vcx, \vcy)$.

Ideally, we would like to learn a hypothesis that minimizes the
\emph{generalization error}\index{Generalization error} $\mcl{E}$:
\begin{equation}
	\label{eq:generalization}
	\mcl{E} = \int_{X \times Y} L(h(\vcx), \vcy) p(\vcx, \vcy) d\vcx d\vcy
\end{equation}
...
discrepancy between the prediction $\hat{\vcy}$ from the true value $\vcy$ and
as such, the best hypothesis is the one that minimizes this integral. Obviously,
it is impossible to evaluate the integral in Equation \ref{eq:generalization}
since we don't have access to infinite data.

The idea is to use the \emph{training error or loss}\index{Training
error}\index{Training loss} as a proxy for the 
